  ...
  return (
    <main className="container mx-auto flex min-h-[calc(100vh-4rem)] max-w-lg items-center px-7 py-12">
      <section className="card-lg w-full p-8">
        ...
        <div className="mt-5 space-y-3">{sessions.map((session)=><div key={session.id} className="rounded-xl border p-4 text-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{session.current?"This device":"Another device"}</p><p className="text-soft mt-1 break-all text-xs">{session.userAgent??"Unknown browser"}</p><p className="text-soft mt-1 text-xs">Last active {new Date(session.lastSeenAt).toLocaleString()}{session.ipAddress?` · ${session.ipAddress}`:""}</p></div>{!session.current?<button type="button" onClick={()=>void revoke(session.id)} className="text-xs underline">Revoke</button>:null}</div></div>)}</div>
      </section>
    </main>
  );
}
