...
function secret(): string {
  const configured = process.env.CUVEE_AUTH_SECRET;
  if (configured) return configured;
  if (process.env.NODE_ENV === "production") {
    throw new Error("CUVEE_AUTH_SECRET must be configured in production");
  }
  return "cuvee-local-demo-secret-change-before-production";
}
...
export function createSessionToken(user: AuthUser): string {
  const payload = Buffer.from(
    JSON.stringify({ sub: user.id, exp: Date.now() + MAX_AGE * 1000 }),
  ).toString("base64url");
