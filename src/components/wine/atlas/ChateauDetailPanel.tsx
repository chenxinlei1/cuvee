  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-5 px-7 py-6">
        ...
        >
          {loading ? t("common.running") : !authReady ? "Checking access…" : canRun ? `${t("common.run_analysis")} · ${chateau.name.replace("Château ", "")}` : "Viewer · read only"}
        </button>
        {authReady&&!canRun?<p className="mt-3 rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-200">当前账号为 Viewer，只能查看已有报告，不能运行 Agent 分析。</p>:null}
