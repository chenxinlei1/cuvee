...
1. Our system's PREDICTED quality score, risk score, and quality band for a past vintage.
...
function outputLanguageInstruction(locale: "en" | "fr" | "zh"): string {
  if (locale === "zh")
    return "OUTPUT LANGUAGE: Simplified Chinese. Write accuracySummary and explanatory quote summaries in professional Chinese. Preserve publication names, château names, URLs, numeric scales, and direct quotations in their original language.";
  if (locale === "fr")
    return "OUTPUT LANGUAGE: French. Write accuracySummary and explanatory quote summaries in professional French. Preserve publication names, château names, URLs, numeric scales, and direct quotations in their original language.";
    ...
    const out = await runTavilyHarness(
      {
        ...
        ...(chateau ? { chateau } : {}),
        ...(!chateau && regionName !== "Bordeaux" ? { query: regionName } : {}),
      },
      { signal },
    );
  ...
  async run(input, ctx) {
    ...
    try {
      ...
      const userMessage = [
        ...
        `Our predicted QUALITY score: ${100 - input.predictedScore}/100 (${predictedBand} quality band)`,
        `Our predicted RISK score: ${input.predictedScore}/100 (inverse of quality)`,
