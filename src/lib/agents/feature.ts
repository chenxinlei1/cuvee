...
- Preserve source filenames from upstream evidence. Never remove a filename that appears in Drivers, Recommendations, or Rationale.
- When upstream evidence contains an internal document filename, the executive summary must cite at least 3 distinct internal facts or metrics when available (for example frost %, rainfall mm, disease level, yield hl/ha, or production change %).
- Do not reduce multiple internal risks to botrytis alone. Reflect frost, mildew/disease, harvest rainfall, and yield/production impact when upstream evidence provides them.
- Treat risk score and quality band as different concepts: risk 40/100 can consistently coexist with a Good quality band.

1. executiveSummary — exactly 2 sentences, max 80 words total.
   Sentence 1: state the verdict (risk band + score) and summarize the evidence; when internal evidence exists, include at least 3 specific internal facts and the source filename.
   Sentence 2: state the most actionable implication for the persona.
...
function outputLanguageInstruction(locale: "en" | "fr" | "zh"): string {
  if (locale === "zh")
    return "OUTPUT LANGUAGE: Simplified Chinese. Translate all headings, prose, actions, table labels, subject lines, and caveats into natural professional Chinese. Keep château names, critic names, filenames, URLs, units, and direct quotations in their original form.";
  if (locale === "fr")
    return "OUTPUT LANGUAGE: French. Translate all headings, prose, actions, table labels, subject lines, and caveats into natural professional French. Keep château names, critic names, filenames, URLs, units, and direct quotations in their original form.";
...
export const featureAgent: SubAgent<FeatureInput, FeatureOutput> = {
  ...
  async run(rawInput, ctx) {
    ...
    const llm = await tryLLM(input, ctx.signal, ctx.locale);
    if (llm) {
      return {
        agent: "feature_agent",
        ok: true,
