...
const Body = z.object({ markdown: z.string().min(1).max(80_000), reportId:z.string().uuid().optional() });
...
function markdownBlocks(markdown: string): Array<Paragraph | Table> {
  ...
  for (let i = 0; i < lines.length; i += 1) {
      ...
      output.push(new Table({
        ...
        rows: data.map((row, rowIndex) => new TableRow({
          ...
          children: row.map((cell) => new TableCell({
            ...
            shading: rowIndex === 0 ? { fill: "F2F4F7", type: ShadingType.CLEAR } : undefined,
    ...
    if (numbered) {
      output.push(new Paragraph({ numbering: { reference: "report-numbering", level: 0 }, children: runs(numbered[1] ?? "") }));
      continue;
    }
...
export async function POST(request: Request) {
  ...
  if(user.role==="viewer"&&(!parsed.data.reportId||!(await canDownloadReport(user,parsed.data.reportId))))return NextResponse.json({error:"Download permission required"},{status:403});

  const word = new Document({
    styles: {
      default: { document: { run: { font: "Arial", size: 22, color: "202124" }, paragraph: { spacing: { after: 140, line: 276 } } } },
      paragraphStyles: [
        { id: "Title", name: "Title", basedOn: "Normal", next: "Normal", run: { font: "Arial", size: 42, bold: true }, paragraph: { spacing: { after: 240 }, keepNext: true } },
        { id: "Heading1", name: "Heading 1", basedOn: "Normal", next: "Normal", run: { font: "Arial", size: 30, bold: true, color: "7A2238" }, paragraph: { spacing: { before: 280, after: 120 }, keepNext: true } },
        { id: "Heading2", name: "Heading 2", basedOn: "Normal", next: "Normal", run: { font: "Arial", size: 25, bold: true, color: "7A2238" }, paragraph: { spacing: { before: 220, after: 100 }, keepNext: true } },
      ],
    },
    numbering: { config: [{ reference: "report-numbering", levels: [{ level: 0, format: LevelFormat.DECIMAL, text: "%1.", alignment: AlignmentType.START, style: { paragraph: { indent: { left: 720, hanging: 360 } } } }] }] },
    sections: [{ properties: { page: { margin: { top: 1440, right: 1440, bottom: 1440, left: 1440 } } }, children: markdownBlocks(parsed.data.markdown) }],
  });
