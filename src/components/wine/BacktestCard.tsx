...
export function BacktestCard({ backtest }: Props) {
  const t = useT();
  // Stored as risk (high = bad); critic ratings are quality (high = good).
  // Convert before rendering so the two columns share one direction.
  const predictedQualityScore = Math.max(0, Math.min(100, 100 - backtest.predictedScore));
  return (
    <article className="card-lg p-7">
      ...
      <div className="mt-6 grid min-w-0 gap-6 md:grid-cols-2">
        {/* Predicted */}
        <div className="min-w-0 rounded-md border border-line bg-surface-2 p-5">
          <p className="kicker">{t("backtest.predicted_quality")}</p>
          <p className="mt-3 font-serif text-5xl font-medium tabular">
            {predictedQualityScore}
            <span className="ml-2 font-sans text-sm font-medium text-soft">/100</span>
          </p>
        ...
        {/* Actual — critics list */}
        <div className="min-w-0 overflow-hidden rounded-md border border-line bg-surface-2 p-5">
          ...
          {backtest.critics.length === 0 ? (
            ...
            <ul className="mt-3 space-y-2.5">
              {backtest.critics.slice(0, 4).map((c, i) => (
                <li
                  key={i}
                  className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-3 text-sm"
                >
                  <span className="min-w-0 break-words text-[10px] font-bold uppercase leading-relaxed tracking-[0.12em] text-soft">
                    {c.source}
                  </span>
