...
export interface AgentContext {
  /** Authenticated owner scope used to isolate memory and private evidence. */
  ownerId?: string;
  region: Pick<Region, "id" | "name" | "parent">;
  timeframe: Timeframe;
  persona: Persona;
  /** Language requested by the active UI for all generated prose. */
  locale: "en" | "fr" | "zh";
  /**
   * Vineyard-side TXT/CSV uploads. The extraction agent retrieves relevant
   * chunks from their bounded text content before the LLM call.
   */
  uploads?: UploadMeta[];
  /** The user's refinement question, also used as the private-RAG query. */
  question?: string;
  /**
