  ...
  return (
    <>
      <div className="shrink-0 border-b border-line px-5 py-4">
        <p className="kicker mb-2">{t("atlas.title")}</p>
        <div className="flex items-baseline gap-2">
          <span className="text-xl font-bold tabular tracking-tight">{REGIONS.length}</span>
          <span className="kicker">{t("atlas.region_count")}</span>
        </div>
      </div>

      <div className="shrink-0 border-b border-line px-5 py-4">
        <p className="kicker mb-2.5">{t("atlas.parent")}</p>
        <div className="grid grid-cols-2 gap-1.5">
          {(["burgundy", "bordeaux"] as const).map((p) => (
            ...
            >
              {p === "burgundy" ? t("atlas.burgundy") : t("atlas.bordeaux")}
