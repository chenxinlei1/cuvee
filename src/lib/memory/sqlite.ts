...
async function getDatabase(): Promise<import("node:sqlite").DatabaseSync | null> {
  ...
  try {
    ...
    `);
    const columns = database.prepare("PRAGMA table_info(analysis_history)").all() as unknown as Array<{ name: string }>;
    if (!columns.some((column) => column.name === "owner_id")) {
      database.exec("ALTER TABLE analysis_history ADD COLUMN owner_id TEXT");
      database.exec("CREATE INDEX IF NOT EXISTS idx_history_owner ON analysis_history(owner_id)");
    }
    return database;
  } catch {
    database = null;
...
export const sqliteMemoryStore: MemoryStore = {
    ...
    try {
      db.prepare(
        `INSERT INTO analysis_history (
          id, owner_id, region_id, chateau, year, persona, trade_persona,
  ...
  async findSimilar(q: FindSimilarQuery): Promise<AnalysisRecord[]> {
    ...
    try {
      ...
      const where: string[] = ["owner_id = ?", "region_id = ?", "persona = ?"];
      const params: Array<string | number> = [q.ownerId, q.regionId, q.persona];
      if (q.chateau) {
